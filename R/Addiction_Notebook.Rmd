...
The following example illustrates estimating the causal effect of smoking on pychological distress:

Primarly, all smokers need to be matched to a non-smoker from the control group. 
```{r}
...
To highlight the difference in the matched and unmatched data set concerning the adjustment of all social dempgraphic variables:
```{r}
...
For inference the results we need to we will need to calculate the cluster-robust standard error and the corresponding confidence interval:
```{r}
