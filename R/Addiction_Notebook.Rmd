...
---
Einleitung: 
Zwar sind Randomisiert Kontrollierte Studien (RCTs) der Goldstandard für kausale Schlussfolgerungen, diese sind in der Suchtforschung aber oftmals aus ethischen Grunden nicht durchführtbar; so beispielsweise bei der Untersuchung der Auswirkung des Rauchens auf Krebserkrankungen. 
Stattdessen werden Beobachtungsdaten aus der realen Welt zunehmend als Grundlage für klinische Entscheidungen und gesundheitspolitische Maßnahmen verwendet. In diesem Beitrag wird der Rahmen für potenzielle Ergebnisse für kausale Schlussfolgerungen vorgestellt und es werden bewährte Methoden der Kausalanalyse für Beobachtungsdaten zusammengefasst. Darunter: Matching, Inverse Probability Weighting (IPW) und Interrupted Time-Series Analysis (ITSA). 
Diese Methoden werden anhand von Beispielen aus der Suchtforschung erläutert sowie die daraus entstandenen Ergebnisse verglichen. 
```{r}
Benötigte Packages und Bibliotheken: 
...
Nicht nötig, die Packages zu erklären oder Libraries zu erklären oder?
```
...
Remoteness: Remoteness of an individual’s residence. 0: major cities; 1: inner regional; 2: outer regional or more remote area
...
#Diese Funktion würde ich einmal illustrieren: Erst Propensity Score für alle Individuen, dann Nearest-Neighbour ein Match auswählen. 
